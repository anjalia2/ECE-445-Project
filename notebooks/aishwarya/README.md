...
![Image description]()
Figure 1: The water-safe temperature sensor we will use.

![Image description]()
Figure 2: The pH sensor we will use.

![Image description]()
Figure 3: The flow meter we will use.
...
![Image description]()
Figure 4: Our initial block diagram


![Image description]()
Figure 5: The initial design of our project idea
...
![Image description]()
Figure 7: The schematic for the fish feeder subsystem that we replicated on a breadboard.
...
![Image description]()
Figure 8: The code for our pH sensor which polls every 30ms
...
We decided we would switch to polling the flow sensor and we made it so that when the if the servo motor was not on and a falling edge was detected in the digital read we would increment a value called NbTopsFan, which counts the amount of pulses we detect in the digital read between the last flow meter reading and the present time. Then, only if it has been 5 seconds since our last flow meter reading, we conver that pulse rate to a liters per hour flow rate and print it out to the serial port. The equation we used for this conversion is based on the original documentation, which is __l_hour = (flow_frequency * 60/7.5)__. Our equation based on our code ended up being flowRate = (NbTopsFan * 60.0 / 7.5). We basically scale up the pulse rate to what it would be in a minute and then divide by the calibration factor of the sensor which is 7.5. When we tested this out, our flow meter was able to output a reading when we blew into it and did not cause issues with out pH output or servo motor rotation. 


![Image description]()
Figure 9: The code for our flow meter sensor
...
Sources:
-the pH sensor docs
-the flow meter interrupt code docs
-the flow meter docs
...
![Image description]()
Figure 10: The code for our grow LEDs and LED driver

Sources:
-the LEDC library references
...
![Image description]()
Figure 11: The old fish feeder CAD model


![Image description]()
Figure 12: My new fish feeder cad model with a press-fit lid
...
![Image description]()
Figure 13: The portion of the water pump code for our demo where the water pump will not turn on for the first 30 seconds
...
![Image description]()
Figure 15: Table of the power subsystem verifications
